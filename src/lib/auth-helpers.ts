...
export async function requireAuth(): Promise<{ ctx: UserContext; error: null } | { ctx: null; error: NextResponse }> {
...
export async function requireManager(): Promise<{ ctx: UserContext; error: null } | { ctx: null; error: NextResponse }> {
  const { ctx, error } = await requireAuth()
  if (error) return { ctx: null, error }
  if (ctx!.role !== 'manager') {
    return { ctx: null, error: NextResponse.json({ error: 'Manager access required' }, { status: 403 }) }
  }
  return { ctx: ctx!, error: null }
}

export async function requireAgent(): Promise<{ ctx: UserContext; error: null } | { ctx: null; error: NextResponse }> {
  const { ctx, error } = await requireAuth()
  if (error) return { ctx: null, error }
  if (!['manager', 'agent'].includes(ctx!.role)) {
    return { ctx: null, error: NextResponse.json({ error: 'Agent access required' }, { status: 403 }) }
  }
  return { ctx: ctx!, error: null }
}

export async function requireAttendant(): Promise<{ ctx: UserContext; error: null } | { ctx: null; error: NextResponse }> {
  const { ctx, error } = await requireAuth()
  if (error) return { ctx: null, error }
  if (!['manager', 'agent', 'attendant'].includes(ctx!.role)) {
    return { ctx: null, error: NextResponse.json({ error: 'Staff access required' }, { status: 403 }) }
  }
  return { ctx: ctx!, error: null }
}

// Convenience booleans (useful in UI components)
