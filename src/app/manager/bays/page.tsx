...
export default function BayMonitorPage() {
  // Get branch_id from /api/auth/me on mount
  ...
  // Staff lookup (simple fetch — in Phase 2 we just show attendant_id, full name comes when staff route is wired to UI)
  ...
  const kpis = [
    {
      label: "Active Bays",
      value: loading ? "—" : `${occupiedCount}/${bays.length}`,
      icon: Warehouse,
      color: "text-blue-600",
      bg: "bg-blue-50",
    },
    {
      ...
      value: loading ? "—" : queueVehicles.length.toString(),
      icon: Car,
    ...
    {
      label: "In-Bay Now",
      value: loading ? "—" : vehicles.filter((v) => v.status === "In-Bay").length.toString(),
      icon: Settings2,
    ...
    },
    {
      ...
      value: loading ? "—" : vehicles.filter((v) => v.status === "Ready").length.toString(),
      icon: CheckCircle2,
    ...
    return (
      <div className="p-8">
        <div className="bg-red-50 border-2 border-dashed border-red-200 rounded-3xl p-6 flex items-center gap-4">
          ...
          <p className="font-bold text-red-900">{authError} — please sign in again.</p>
        </div>
      </div>
    );
  }
      ...
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">
            ...
            {!loading && (
              <span className="ml-2 text-emerald-500 font-bold text-xs uppercase">
                ● Realtime
              </span>
            )}
          </p>
          ...
          <Card key={i} className="border-none shadow-sm rounded-[2rem] overflow-hidden group">
            <CardContent className="p-8">
              <div className="flex justify-between items-start mb-6">
                <div
                  className={`size-12 ${kpi.bg} ${kpi.color} rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform`}
                >
          ...
          {loading
            ...
            : bays.map((bay) => {
                const vehicle = vehicles.find(
                  (v) => v.bay_id === bay.id && v.status !== "Completed"
                );
                const attendantName = vehicle?.attendant_id
                  ? staffMap[vehicle.attendant_id] ?? "Assigned"
                  : null;
                  ...
                  >
                    <CardHeader className="p-8 pb-4">
                      <div className="flex justify-between items-center mb-4">
                        <Badge
                          variant="outline"
                          className="rounded-lg px-3 py-1 font-bold text-[10px] uppercase tracking-wider bg-white"
                        >
                          {bay.name}
                        </Badge>
                        <Badge
                      ...
                      {bay.status === "Occupied" && vehicle ? (
                        <>
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-mono font-black tracking-widest text-slate-900">
                                {vehicle.plate}
                              </h3>
                              <div className="flex items-center gap-2 text-xs text-slate-400 font-bold uppercase mt-1">
                                <User className="size-3" />
                                {attendantName ?? "Assigning..."}
                              </div>
                            </div>
                          </div>
          ...
          <CardContent className="p-4 space-y-4">
            {loading ? (
              ...
              queueVehicles.map((v, i) => (
                <div
                  key={v.id}
                  className="p-4 bg-slate-50 rounded-3xl border border-slate-100 flex items-center gap-4 group hover:bg-white hover:shadow-lg transition-all"
                >
                  ...
                  <Button
                    size="icon"
                    variant="ghost"
                    className="rounded-xl group-hover:bg-primary group-hover:text-white transition-colors"
                  >
